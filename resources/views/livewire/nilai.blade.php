                ...
                <div class="flex flex-col items-center">
                    @if(isset($leaderboard[1]))
                        <div class="w-20 h-20 rounded-full border-4 border-gray-300 dark:border-gray-700 overflow-hidden mb-2">
                            <img src="{{ $leaderboard[1]->user->profile_photo_url ?? 'https://ui-avatars.com/api/?name='.urlencode($leaderboard[1]->user->name) }}" class="w-full h-full object-cover">
                        </div>
                ...
                <div class="flex flex-col items-center">
                    @if(isset($leaderboard[0]))
                        <div class="w-24 h-24 rounded-full border-4 border-yellow-400 overflow-hidden mb-2 ring-4 ring-yellow-200 dark:ring-yellow-700">
                            <img src="{{ $leaderboard[0]->user->profile_photo_url ?? 'https://ui-avatars.com/api/?name='.urlencode($leaderboard[0]->user->name) }}" class="w-full h-full object-cover">
                        </div>
                ...
                <div class="flex flex-col items-center">
                    @if(isset($leaderboard[2]))
                        <div class="w-20 h-20 rounded-full border-4 border-amber-700 dark:border-amber-900 overflow-hidden mb-2">
                            <img src="{{ $leaderboard[2]->user->profile_photo_url ?? 'https://ui-avatars.com/api/?name='.urlencode($leaderboard[2]->user->name) }}" class="w-full h-full object-cover">
                        </div>
                        ...
                        @foreach($leaderboard as $i => $row)
                            <tr class="{{ $row->user_id == $user_id ? 'bg-indigo-50 dark:bg-indigo-900 font-semibold' : 'dark:text-gray-100' }}">
                                ...
                                <td class="py-2 px-3 flex items-center gap-2">
                                    <img src="{{ $row->user->profile_photo_url ?? 'https://ui-avatars.com/api/?name='.urlencode($row->user->name) }}" class="w-8 h-8 rounded-full" alt="">
                                    {{ $row->user->name }}
